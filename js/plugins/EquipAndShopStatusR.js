...
Imported['EquipAndShopStatusR'] = 1.03;

/*:
 * @plugindesc ver1.03/装備&ショップシーンのステータス表示を改造します。
 * @author Yana
 *
 * @param 【用語の設定】
 *
    ...
    Window_EquipStatus.prototype.execRefresh = function() {
        if (this._callRefresh){
            if (this.checkBlt()){
                ...
                if (!this._initStandPicture){ this.loadStandPictures() }
            }
        }
    };
    ...
    Window_EquipStatus.prototype.loadStandPictures = function(){
        $gameParty.members().forEach(function(actor) {
            if (actor && actor.pictureName()){
                var bitmap = ImageManager.loadPicture(actor.pictureName());
                bitmap = null;
            }
        });
    };
